...
def create_super_trend(con):
    data=pd.read_sql_query("SELECT * FROM OneMinCandle",con)
...
while (datetime.datetime.now().hour!=16 and datetime.datetime.now().minute!=30):
    create_super_trend(con)
    print("super trend inserted")
con.close()
